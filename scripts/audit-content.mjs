...
if (files.length < 51) errors.push(`Expected at least 51 guides, found ${files.length}.`);
...
for (const file of files) {
  ...
  if (/\b(?:API API|API interface|interface API)\b/i.test(raw)) errors.push(`${file}: duplicated API wording remains.`);
  ...
  if (/https?:\/\/(?:www\.|api\.)?gugudata\.com\b/i.test(raw)) errors.push(`${file}: contains a GuGuData .com link.`);
  if (/\]\(\/(?:guides|diagrams)\//i.test(raw)) errors.push(`${file}: root-relative site link omits the Pages base path.`);
  ...
  const unsafeKey = raw.match(/appkey=([A-Za-z0-9_-]{12,})/i)?.[1]
    ?? raw.match(/APPKEY\s*=\s*["']([A-Za-z0-9_-]{12,})["']/)?.[1];
